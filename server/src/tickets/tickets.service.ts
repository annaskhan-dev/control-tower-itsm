...
export class TicketsService {
  ...
  constructor(
    ...
    @InjectModel(SlaConfig.name)
    private slaConfigModel: Model<SlaConfigDocument>,
  ...
  private getEffectiveResolver(
    ...
    if (
      payloadSub &&
      typeof payloadSub === 'string' &&
      payloadSub !== 'Unassigned' &&
      payloadSub.trim() !== '' &&
      payloadSub !== 'null'
    ) {
    ...
    if (
      ticketSub &&
      typeof ticketSub === 'string' &&
      ticketSub !== 'Unassigned' &&
      ticketSub.trim() !== '' &&
      ticketSub !== 'null'
    ) {
    ...
    if (
      fallbackSub &&
      typeof fallbackSub === 'string' &&
      fallbackSub !== 'Unassigned' &&
      fallbackSub.trim() !== '' &&
      fallbackSub !== 'null'
    ) {
  ...
  async create(
    ...
    try {
      const resolvedCompanyId =
        companyId || createTicketDto.companyId || 'openport123';
      ...
      if (isSalesRole) {
        if (createTicketDto.category) {
          throw new ForbiddenException(
            'Sales Ops are not allowed to choose the category.',
          );
        }
        if (createTicketDto.issueType) {
          throw new ForbiddenException(
            'Sales Ops are not allowed to choose the incident type.',
          );
        }
      }

      const restrictedAssignmentKeywords = [
        'transporter',
        'sales',
        'shipper',
        'ops',
      ...
      if (
        ...
        createTicketDto.assignee !== 'Unassigned'
      ) {
        const lowerAssignee = createTicketDto.assignee.toLowerCase();
        const isRestricted = restrictedAssignmentKeywords.some((keyword) =>
          lowerAssignee.includes(keyword),
        );
        if (isRestricted) {
      ...
      if (
        createTicketDto.subAssignment &&
        createTicketDto.subAssignment !== 'Unassigned' &&
        createTicketDto.subAssignment !== ''
      ) {
        const lowerSub = createTicketDto.subAssignment.toLowerCase();
        const isRestrictedSub = restrictedAssignmentKeywords.some((keyword) =>
      ...
      const isAssigned =
        createTicketDto.assignee && createTicketDto.assignee !== 'Unassigned';
      const isSubAssigned =
        createTicketDto.subAssignment &&
        createTicketDto.subAssignment !== 'Unassigned' &&
        createTicketDto.subAssignment !== '';
      ...
      if (isAssigned && isSubAssigned) {
        if (
          createTicketDto.assignee?.trim().toLowerCase() ===
          createTicketDto.subAssignment?.trim().toLowerCase()
        ) {
          throw new BadRequestException(
            'The assignee and sub-assignee cannot be the same person.',
          );
        }
      }
      ...
      if (category) {
        const slaConfig = await this.slaConfigModel
          .findOne({ category, companyId: resolvedCompanyId })
          .exec();
      ...
      const resolvedUserName =
        userName && userName !== 'User' ? userName : 'Ali';
      const ticketGenerator =
        createTicketDto.generator ||
        `${resolvedUserName} (${userRole || 'Super Admin'})`;
      ...
      const ticketData = {
        ...
        ticketId:
          (createTicketDto as any).ticketId ||
  ...
  async update(
    ...
    if (
      isSalesRole ||
      restrictedUpdateRoles.some((r) => normalizedRole.includes(r))
    ) {
      ...
      if (updateTicketDto.issueType !== undefined) {
        throw new ForbiddenException(
          'You are not allowed to update Incident Type.',
        );
      }
      ...
      if (updateTicketDto.assignee !== undefined) {
        if (normalizedRole === 'operator') {
          if (currentUserName && updateTicketDto.assignee !== currentUserName) {
            throw new ForbiddenException(
              'Operators can only assign tickets to themselves.',
            );
          }
        } else {
          throw new ForbiddenException(
            'You are not allowed to update Assignee.',
          );
        }
      }
    }
    ...
    if (
      updateTicketDto.subAssignment &&
      updateTicketDto.subAssignment !== '' &&
      updateTicketDto.subAssignment !== 'Unassigned'
    ) {
    ...
    const targetAssignee =
      updateTicketDto.assignee !== undefined
        ? updateTicketDto.assignee
        : existingTicket.assignee;
    const targetSubAssignment =
      updateTicketDto.subAssignment !== undefined
        ? updateTicketDto.subAssignment
        : existingTicket.subAssignment;

    const isTargetAssigned =
      targetAssignee &&
      targetAssignee !== 'Unassigned' &&
      targetAssignee !== '';
    const isTargetSubAssigned =
      targetSubAssignment &&
      targetSubAssignment !== 'Unassigned' &&
      targetSubAssignment !== '' &&
      targetSubAssignment !== null;

    if (isTargetAssigned && isTargetSubAssigned) {
      if (
        targetAssignee?.trim().toLowerCase() ===
        targetSubAssignment?.trim().toLowerCase()
      ) {
        throw new BadRequestException(
          'The assignee and sub-assignee cannot be the same person.',
        );
    ...
    const isAlreadyResolved = ['resolved', 'completed', 'done'].includes(
      currentStatus,
    );

    if (isAlreadyResolved) {
      const isChangingCategory =
        updateTicketDto.category !== undefined &&
        updateTicketDto.category !== existingTicket.category;
      const isChangingSubAssignment =
        updateTicketDto.subAssignment !== undefined &&
        updateTicketDto.subAssignment !== existingTicket.subAssignment;

      if (isChangingCategory || isChangingSubAssignment) {
        throw new BadRequestException(
          'Cannot modify category or sub-assignment once a ticket is resolved.',
        );
      }
    }

    if (updateTicketDto.status !== undefined && existingTicket.subAssignment) {
      const isManagerOrAdmin = ['manager', 'super_admin', 'admin'].some((r) =>
      ...
      if (isPrimaryAssignee && !isManagerOrAdmin) {
        throw new ForbiddenException(
          'Primary assignees cannot change the ticket status once a sub-assignee is assigned.',
        );
      }
    }
    ...
    if (updateData.category !== undefined) {
      if (updateData.category) {
        const slaConfig = await this.slaConfigModel
          .findOne({ category: updateData.category, companyId })
          .exec();
        const hoursAllowed = slaConfig ? slaConfig.hours : 24;
        updateData.slaDeadline = new Date(
          Date.now() + hoursAllowed * 60 * 60 * 1000,
        );
        if (slaConfig && slaConfig.priority) {
    ...
    if (updateData.assignee !== undefined) {
      const isActuallyAssigned =
        updateData.assignee !== 'Unassigned' && updateData.assignee !== '';
    ...
    if (updateData.subAssignment !== undefined) {
      const isActuallySubAssigned =
        updateData.subAssignment !== 'Unassigned' &&
        updateData.subAssignment !== '' &&
        updateData.subAssignment !== null;

      if (isActuallySubAssigned) {
        const isNewSubAssignment =
          updateData.subAssignment !== existingTicket.subAssignment;
        if (isNewSubAssignment || !existingTicket.subAssignmentAt) {
          updateData.subAssignmentAt = updateData.subAssignmentAt
            ? new Date(updateData.subAssignmentAt)
            : new Date();
        } else {
    ...
    if (
      ...
      isNewResolved = ['resolved', 'completed', 'done'].includes(
    ...
    const updatedTicket = await this.ticketModel
      .findOneAndUpdate({ ...baseQuery, companyId }, updateData, {
        new: true,
        runValidators: true,
      })
      .exec();
    ...
    try {
      this.logger.debug(
        `[Email Debug] Starting email dispatch check for ticket ID: ${updatedTicket._id}`,
      );
      ...
      const resolveUserObj = async (identifier: string | null | undefined) => {
        ...
        this.logger.debug(
          `[Email Debug] Searching user model for identifier: "${identifier}" in company: "${companyId}"`,
        );
        ...
        if (foundUser) {
          this.logger.debug(
            `[Email Debug] Found user in DB: ${JSON.stringify(foundUser)}`,
          );
          return foundUser;
        }

        this.logger.warn(
          `[Email Debug] User not found in DB for "${identifier}".`,
        );
        return null;
      };
      ...
      if (updatedTicket.assignee && updatedTicket.assignee !== 'Unassigned') {
        this.logger.debug(
          `[Email Debug] Processing assignee change email for: "${updatedTicket.assignee}"`,
        );
        const newAssigneeObj = await resolveUserObj(updatedTicket.assignee);

        if (newAssigneeObj && newAssigneeObj.email) {
          if (
            this.emailNotificationService &&
            typeof this.emailNotificationService
              .sendPrimaryAssigneeChangedEmail === 'function'
          ) {
            await this.emailNotificationService.sendPrimaryAssigneeChangedEmail(
              newAssigneeObj.email,
              updatedTicket.ticketId || updatedTicket._id.toString(),
              newAssigneeObj.name || updatedTicket.assignee,
            );
            this.logger.debug(
              `[Email Debug] sendPrimaryAssigneeChangedEmail executed successfully.`,
            );
          } else {
            this.logger.error(
              `[Email Debug] sendPrimaryAssigneeChangedEmail is not a function or service is missing.`,
            );
          }
        } else {
          this.logger.warn(
            `[Email Debug] Skipped primary assignee email: No registered email address found in DB for user "${updatedTicket.assignee}"`,
          );
        }
      }

      if (
        updatedTicket.subAssignment &&
        updatedTicket.subAssignment !== 'Unassigned'
      ) {
        const subAssigneeObj = await resolveUserObj(
          updatedTicket.subAssignment,
        );

        if (
          subAssigneeObj &&
          subAssigneeObj.email &&
          typeof this.emailNotificationService.sendEmail === 'function'
        ) {
          const subject = `[Control Tower] Sub-Assignment Added: ${updatedTicket.ticketId || updatedTicket._id}`;
          const htmlBody = `
            <h3>Hello ${subAssigneeObj.name || 'User'},</h3>
            <p>You have been given a sub-assignment on ticket <b>${updatedTicket.title}</b> (${updatedTicket.ticketId}).</p>
            <p><b>Primary Assignee:</b> ${updatedTicket.assignee}</p>
            <br/>
      ...
      if (oldSlaStatus !== 'Breached' && newSlaStatus === 'Breached') {
        if (
          typeof this.emailNotificationService.sendBreachEmailToManager ===
          'function'
        ) {
          await this.emailNotificationService.sendBreachEmailToManager(
            updatedTicket,
          );
        }
      }
      ...
      this.logger.error(
        `[Email Debug Error] Failed to dispatch email notifications: ${emailErr.message} stack: ${emailErr.stack}`,
      );
    }

    if (isNewResolved && !isAlreadyResolved) {
      const targetUser = this.getEffectiveResolver(
        updatedTicket,
        existingTicket,
        updateTicketDto,
      );
      ...
      if (targetUser && targetUser !== 'Unassigned') {
        try {
          ...
          this.logger.warn(
            `Failed to increment user resolved metrics for ${targetUser}: ${counterErr.message}`,
          );
        }
      }
    }
  ...
  async findAll(
    ...
    const normalizedRole = (userRole || '').replace(/\s+/g, '_').toLowerCase();
    const isManagerOrAdmin = ['manager', 'super_admin', 'admin'].includes(
      normalizedRole,
    );
    ...
    const isGenericRole = genericPlaceholders.some(
      (p) => normalizedRole.includes(p) || trimmedUserName.includes(p),
    );
    const isGenericName =
      !userName ||
      genericPlaceholders.includes(trimmedUserName) ||
      isGenericRole;
    ...
    return tickets.map((t: any) => {
      ...
      const hasSubAssignment =
        ticketObj.subAssignment &&
        ticketObj.subAssignment !== 'Unassigned' &&
        ticketObj.subAssignment !== '';
  ...
  async getStats(companyId: string, userRole?: string, userName?: string) {
    ...
    if (userRole && userName) {
      ...
      const isGenericName =
        genericPlaceholders.includes(trimmedUserName) || isGenericRole;
      if (!isManagerOrAdmin && !isGenericName) {
        const cleanName = userName.includes('@')
          ? userName.split('@')[0]
          : userName;
    ...
    return {
      ...
      open: tickets.filter((t) => (t.status || '').toLowerCase() === 'open')
        .length,
  ...
  async removeSlaConfig(id: string, companyId: string) {
    return await this.slaConfigModel
