...
const normalizeTicket = (t, now) => {
  ...
  const creatorName =
    ...
    rawCreator.trim() !== "" &&
    rawCreator.toLowerCase() !== "operator"
      ? rawCreator.trim()
...
export const Dashboard = ({ tickets: propTickets = [] }) => {
  ...
  const normalizedTickets = useMemo(
    () => tickets.map((t) => normalizeTicket(t, now)),
    [tickets, now],
  );
  ...
  const statusCounts = useMemo(() => {
    ...
    const unassigned = normalizedTickets.filter(
      (t) => t.assigneeName.toLowerCase() === "unassigned",
    ).length;
  ...
  const filteredTickets = useMemo(() => {
    return normalizedTickets.filter((t) => {
      ...
      if (statusFilter === "open") matchesStatus = !t.isResolved;
        ...
        matchesStatus = t.assigneeName.toLowerCase() === "unassigned";

      const matchesPriority =
        priorityFilterTab === "all"
          ? true
          : t.priority.toLowerCase() === priorityFilterTab.toLowerCase();
  ...
  const operatorResolutionStats = useMemo(() => {
    ...
    return {
      ...
      byOperator: Object.entries(map).map(([operator, count]) => ({
        operator,
        count,
      })),
    };
  ...
  const stats = useMemo(
    () => ({
      ...
      slaRisk: normalizedTickets.filter((t) =>
        ["Breached", "At Risk"].includes(t.slaStatus),
      ).length,
    }),
  ...
  const chartData = useMemo(() => {
    ...
    return {
      priority: [
        {
          name: "Critical",
          count: normalizedTickets.filter((t) => t.priority === "Critical")
            .length,
        },
        {
          name: "High",
          count: normalizedTickets.filter((t) => t.priority === "High").length,
        },
        {
          name: "Medium",
          count: normalizedTickets.filter((t) => t.priority === "Medium")
            .length,
        },
        {
          name: "Low",
          count: normalizedTickets.filter((t) => t.priority === "Low").length,
        },
      ],
      type: [
        {
          name: "Request",
          value: normalizedTickets.filter(
            (t) =>
              t.category.toLowerCase().includes("request") ||
              t.category.toLowerCase().includes("dispatch"),
          ).length,
          color: "#3b82f6",
          details: "Standard requests & dispatches",
        },
        {
          name: "Problem",
          value: normalizedTickets.filter(
            (t) =>
              ...
              t.category.toLowerCase().includes("fleet"),
          ).length,
          color: "#f59e0b",
          details: "Fleet & operational issues",
        },
      ].filter((d) => d.value > 0),
      sla: [
        {
          name: "On Track",
          value: normalizedTickets.filter((t) => t.slaStatus === "On Track")
            .length,
          color: "#10b981",
          details: "Meeting target timelines",
        },
        {
          name: "At Risk",
          value: normalizedTickets.filter((t) => t.slaStatus === "At Risk")
            .length,
          color: "#f59e0b",
          details: "Approaching deadline window",
        },
        {
          name: "Breached",
          value: normalizedTickets.filter((t) => t.slaStatus === "Breached")
            .length,
          color: "#ef4444",
          details: "Deadline expired uncompleted",
        },
      ].filter((d) => d.value > 0),
      trend: trend,
    };
    ...
    <div className="w-full max-w-7xl mx-auto flex flex-col gap-6 font-sans text-slate-800 p-4 sm:p-6 overflow-x-hidden">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Real-time ticketing lifecycle, assignment timelines, and SLA
            monitoring
          </p>
        </div>
        <button
        ...
        {[
          ...
          >
            <div>
              ...
              <h3
                className={`text-2xl font-bold ${item.color === "rose" ? "text-rose-600" : item.color === "emerald" ? "text-emerald-600" : "text-slate-900"} mt-1`}
              >
                {item.val}
              </h3>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="p-5 bg-white border border-slate-200/80 rounded-2xl shadow-xs flex flex-col justify-between">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            <h4 className="text-xs font-bold text-slate-700 uppercase tracking-wider flex items-center gap-2">
              <UserCheck size={16} className="text-blue-600" /> Tickets by
              Generator / Source
            </h4>
          ...
          <div className="py-3 flex flex-col gap-2 max-h-40 overflow-y-auto">
            {generatorBreakdown.map((gen, idx) => (
              ...
              >
                ...
                <span className="font-bold bg-blue-50 text-blue-700 px-2.5 py-0.5 rounded-full shrink-0">
                  {gen.count} tickets
                </span>
              </div>
            ))}
        ...
        <div className="p-5 bg-white border border-slate-200/80 rounded-2xl shadow-xs flex flex-col justify-between">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            <h4 className="text-xs font-bold text-slate-700 uppercase tracking-wider flex items-center gap-2">
              <CheckCircle2 size={16} className="text-emerald-600" /> Operator
              Resolutions
            </h4>
            <span className="text-xs font-bold bg-emerald-50 text-emerald-700 px-2 py-0.5 rounded-md">
              Resolved: {operatorResolutionStats.totalResolved}
            </span>
          </div>
          <div className="py-3 flex flex-col gap-2 max-h-40 overflow-y-auto">
            {operatorResolutionStats.byOperator.map((op, idx) => (
              ...
              >
                ...
                <span className="font-bold bg-emerald-50 text-emerald-700 px-2.5 py-0.5 rounded-full shrink-0">
                  {op.count} resolved
                </span>
              </div>
            ))}
            {operatorResolutionStats.byOperator.length === 0 && (
              <p className="text-xs text-slate-400 italic text-center py-2">
                No resolved tickets by operators yet
              </p>
            )}
          </div>
          <div className="text-[10px] text-slate-400 pt-2 border-t border-slate-100 italic">
        ...
        <div className="p-5 bg-white border border-slate-200/80 rounded-2xl shadow-xs flex flex-col justify-between">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            <h4 className="text-xs font-bold text-slate-700 uppercase tracking-wider flex items-center gap-2">
              <ShieldAlert size={16} className="text-amber-600" /> Priority &
              Resolution
            </h4>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="p-5 border border-slate-200/80 rounded-2xl bg-white shadow-xs hover:shadow-md transition-shadow duration-200 flex flex-col justify-between h-64">
          ...
          <div className="pt-2 border-t border-slate-100 flex justify-between text-[10px] text-slate-500 font-medium">
            <span>
              P1/Critical:{" "}
              {chartData.priority.find((p) => p.name === "Critical")?.count ||
                0}
            </span>
        ...
        <div className="p-5 border border-slate-200/80 rounded-2xl bg-white shadow-xs hover:shadow-md transition-shadow duration-200 flex flex-col justify-between h-64">
          ...
          <div className="pt-2 border-t border-slate-100 flex justify-between text-[10px] text-slate-500 font-medium">
            <span>
              Peak: {Math.max(...chartData.trend.map((d) => d.tickets), 0)}{" "}
              tix/day
            </span>
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">
                Manage, track, and filter operational support records
              </p>
            </div>
            ...
            <div className="flex items-center gap-1.5 overflow-x-auto w-full sm:w-auto pb-1 sm:pb-0">
              {[
                ...
                >
                  <span>{tab.label}</span>
                  <span
                    className={`px-1.5 py-0.2 rounded-full text-[10px] ${statusFilter === tab.id ? "bg-slate-700 text-white" : "bg-slate-100 text-slate-600"}`}
                  >
                    {tab.count}
                  </span>
                </button>
            ...
            <tbody className="divide-y divide-slate-100 text-xs text-slate-600">
              ...
              {filteredTickets.length === 0 && (
                <tr>
                  <td
                    colSpan="8"
                    className="py-16 text-center text-sm text-slate-400 italic"
                  >
                    No matching tickets found. Try adjusting your search query
                    or filter settings.
                  </td>
                </tr>
              )}
