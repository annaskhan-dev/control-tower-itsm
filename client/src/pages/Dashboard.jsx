...
/**
 * Unified Normalization Engine: Ensures 100% data and formatting parity with your main Ticket List view.
 */
const normalizeTicket = (t, now) => {
  ...
  // Category vs Type mapping parity:
  // In your Ticket List view, the column header is "CATEGORY" but displays values like "Dispatch & Operations", "Fleet Management", "Request", "Problem", etc.
  ...
  // 1. Assignment Time (Hours & Minutes format matching list view e.g. "0h 3m", "0h 37m")
  ...
  // FIXED: Formatting engine that outputs "0h Xm" format to precisely match your Ticket List view
  const formatDurationFullHours = (ms) => {
    if (ms === null || ms === undefined || isNaN(ms)) return null;
    const hours = Math.floor(ms / (1000 * 60 * 60));
    const mins = Math.floor((ms % (1000 * 60 * 60)) / (1000 * 60));
    
    if (hours === 0 && mins === 0) return "0h 0m";
    return `${hours}h ${mins}m`; // Perfectly matches "0h 3m", "0h 37m", "0h 5m"
  };
  ...
  return {
    ...
    assignmentTimeFormatted: formatDurationFullHours(assignmentTimeMs),
    slaTimeFormatted: formatDurationFullHours(slaTimeMs),
    subAssignmentTimeFormatted: subAssignmentTimeMs !== null ? formatDurationFullHours(subAssignmentTimeMs) : null,
    finalResolutionTimeFormatted: isResolved ? formatDurationFullHours(finalResolutionTimeMs) : null,
  };
};
...
export const Dashboard = ({ tickets: propTickets = [] }) => {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto flex flex-col gap-6 font-sans text-slate-800 p-6">
      ...
      {/* Professional Lifecycle & Timeline Breakdown Table - Perfectly aligned with your main ticket list columns */}
            ...
            <tbody className="divide-y divide-slate-100 text-xs font-medium text-slate-700">
              {normalizedTickets.map((t, index) => (
                <tr key={t.id || index} className="hover:bg-slate-50/60 transition-colors group">
                  ...
                  {/* Assignment Time (Pill with "0h Xm" format matching image references) */}
                  ...
                  {/* SLA Active Time (Pill with blue border and blue text e.g. "0h 5m", "1h 3m") */}
                  <td className="py-4 px-4 whitespace-nowrap">
